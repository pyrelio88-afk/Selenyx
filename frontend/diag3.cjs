...
srv.listen(4178,'127.0.0.1',async()=>{
  let b;try{
    ...
    await pg.goto('http://127.0.0.1:4178/',{waitUntil:'domcontentloaded'});
    ...
    await pg.evaluate(()=>{const all=Array.from(document.querySelectorAll('button,a,li,[role="button"],div'));const t=all.find(e=>e.textContent.trim()==='文献库');if(t)t.click();});
    await sleep(900);
    // click data row
    const rowClicked=await pg.evaluate(()=>{const rows=Array.from(document.querySelectorAll('tr,[role="row"]'));const r=rows.find(x=>x.textContent.includes('Selenyx'));if(r){r.click();return true}return false;});
    await sleep(700);
    const info=await pg.evaluate(()=>({getfull:document.querySelectorAll('button').length&&Array.from(document.querySelectorAll('button')).filter(b=>b.textContent.includes('获取全文')).length}));
    console.log('rowClicked=',rowClicked,'getfull=',JSON.stringify(info));
    await pg.screenshot({path:'/home/gem/.aily/workdir/task_7669744666866224081/artifacts/diag3.png'});
