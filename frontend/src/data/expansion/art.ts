...
export const ART_EXTRA: DisciplineExpansion = {
  glossary: [
    ...
    { term: '后印象派', termEn: 'Post-Impressionism', definition: '19世纪末从印象派发展而来的艺术运动。区别于印象派对光影的客观记录，后印象派强调主观感受和情感表达。代表：塞尚（结构化几何——立体主义先驱）、梵高（表现性色彩和笔触）、高更（象征性原始主义）。', category: '西方美术史' },
    ...
    { term: '皴法', termEn: 'Texture Stroke Technique', definition: '中国画表现山石树木质感和体积感的笔法。主要皴法：披麻皴（董源/江南山峦柔缓）、斧劈皴（李唐/北方山石硬峭）、雨点皴（范宽）、折带皴（倪瓒）、解索皴（王蒙）。皴法是判断画家风格和流派归属的重要依据。', category: '中国美术史' },
    { term: '留白', termEn: 'Blank Space / Negative Space', definition: '中国画构图中有意留出的空白，\'计白当黑\'——空白不是无而是有意境的表达。留白可以表现云雾/水面/天空，更可以营造\'此时无声胜有声\'的诗意空间。马远、夏圭（\'马一角夏半边\'）以大量留白著称。', category: '中国美术史' },
    { term: '散点透视', termEn: 'Scatter Perspective', definition: '中国画特有的透视方法——不受固定视点限制，将不同角度不同距离的景物组织在同一画面中。与西方焦点透视（一个消失点）不同，散点透视可\'移步换景\'，适合表现长卷式构图。代表：张择端《清明上河图》（5米长卷）、王希孟《千里江山图》。', category: '中国美术史' },
    ...
    { term: '构图', termEn: 'Composition', definition: '画面中各视觉元素的组织安排。基本原则：三分法（将画面分为九宫格、主体置于交叉点）、引导线（利用线条引导视线）、对称与不对称平衡、前景中景背景层次。构图决定画面的视觉节奏和叙事逻辑。', category: '技法理论' },
    ...
    { term: '工笔与写意', termEn: 'Gongbi and Xieyi', definition: '中国画的两种基本画法。工笔：精勾细染、层层敷色、工整细致（如宋徽宗瑞鹤图）。写意：简练概括、笔墨淋漓、重在神韵（如徐渭墨葡萄、八大山人鱼鸟）。工笔重形似，写意重意境——\'似与不似之间\'（齐白石）。', category: '中国美术史' },
    { term: '书法五体', termEn: 'Five Scripts of Chinese Calligraphy', definition: '中国书法的五种主要字体：篆书（大篆/小篆，秦代统一）、隶书（汉代通行，蚕头燕尾）、楷书（唐代鼎盛，方正端庄）、行书（介于楷草之间，王羲之兰亭序）、草书（章草/今草/狂草，张旭怀素）。五体演变反映了中国文字简化和艺术化的双重进程。', category: '书法' },
