...
const CHINA_SOURCES = Object.freeze([
  ...
  defSource({
    ...
    name: 'ChinaXiv 预印本',
    nameEn: 'ChinaXiv',
    homeUrl: 'http://chinaxiv.org',
    access: 'free',
    nativeApi: { available: false, note: '无公开检索 API' },
    searchLink: {
      available: true,
      buildUrl: (q) => `http://chinaxiv.org/user/search.htm?searchText=${enc(q)}`,
      note: '跳转 ChinaXiv 检索',
    },
  ...
  defSource({
    ...
    name: '国家哲学社会科学文献中心',
    nameEn: 'NCPSSD',
    homeUrl: 'https://www.ncpssd.org',
    access: 'free',
    nativeApi: { available: false, note: '无公开 API' },
    searchLink: {
      available: true,
      buildUrl: (q) => `https://www.ncpssd.org/Literature/articleList.aspx?searchText=${enc(q)}`,
      note: '跳转国家哲社文献中心',
    },
    note: '社科领域国家级免费平台',
  }),
  defSource({
    ...
    searchLink: {
      available: true,
      buildUrl: (q) => `https://www.sinomed.ac.cn/zh/search?q=${enc(q)}`,
      note: '跳转 SinoMed，可能需机构权限',
    },
  ...
  defSource({
    ...
    searchLink: {
      available: true,
      buildUrl: (q) => `https://kns.cnki.net/kns8s/defaultresult/index?kw=${enc(q)}`,
      note: '跳转知网检索，需机构或个人账号',
    },
  ...
  defSource({
    ...
    searchLink: {
      available: true,
      buildUrl: (q) => `https://qikan.cqvip.com/Qikan/Search/Index?key=${enc(q)}`,
      note: '跳转维普检索，需账号',
    },
    note: '商业平台，仅检索跳转',
  }),
